...
def solve_adaptive(f: Callable, x0: float, y0: np.ndarray, x_end: float, rtol: float = 1e-5, atol: float = 1e-8, stiff: bool = False) -> Tuple[np.ndarray, np.ndarray, str]:
    """Adaptive solver utilizing RK45 (explicit) or BDF (implicit) for stiff equations."""
    solver_method = 'BDF' if stiff else 'RK45'
    sol = solve_ivp(lambda t, y: f(t, y), [x0, x_end], y0, method=solver_method, rtol=rtol, atol=atol)
    status = f"SUCCESS ({solver_method})" if sol.success else f"FAILED ({sol.message})"
