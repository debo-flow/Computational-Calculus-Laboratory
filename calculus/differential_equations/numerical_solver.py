...
def solve_fixed_step(f: Callable, x0: float, y0: np.ndarray, x_end: float, h: float, method: str = 'RK4') -> Tuple[np.ndarray, np.ndarray, str]:
    """Generates numerical trajectory over [x0, x_end] using specified stepper."""
    ...
    methods = {
        'Euler': euler_step, 
        'Heun': heun_step, 
        'RK2 (Midpoint)': rk2_midpoint_step, 
        'RK3': rk3_step, 
        'RK4': rk4_step
    }
    ...
    for i in range(steps):
        try:
            ...
            if np.any(np.isnan(y_vals[i+1])) or np.any(np.isinf(y_vals[i+1])):
                return x_vals[:i+1], y_vals[:i+1], "FAILED (Numerical Instability/Blow-up)"
        except Exception as e:
            return x_vals[:i+1], y_vals[:i+1], f"FAILED (Evaluation Error: {e})"
    return x_vals, y_vals, "SUCCESS"

def solve_adaptive(f: Callable, x0: float, y0: np.ndarray, x_end: float, rtol: float = 1e-5, atol: float = 1e-8, stiff: bool = False) -> Tuple[np.ndarray, np.ndarray, dict]:
    """Adaptive solving using SciPy with explicit stiffness control."""
    solver_method = 'BDF' if stiff else 'RK45'
    sol = solve_ivp(lambda t, y: f(t, y), [x0, x_end], y0, method=solver_method, rtol=rtol, atol=atol)
    
    status_info = {
        "Status": "SUCCESS" if sol.success else f"FAILED ({sol.message})",
        "Method": solver_method,
        "Function Evaluations": sol.nfev,
        "Accepted Steps": len(sol.t)
    }
    return sol.t, sol.y.T, status_info
